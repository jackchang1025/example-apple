...
{
    const string PROXY_HOST = 'http://proxy.stormip.cn';
    ...
    private array $defaultConfig = [
        ...
        'area'    => "-1",// 全球地区code 例如：美国 area-US  点击查看
    ...
    public function getProxy(Option $option): ProxyResponse
    {
        ...
        return new ProxyResponse([
            ...
            'auth'     => [$username, $config['password']],
            'host'     => self::PROXY_HOST,
            'port'     => self::HTTP_PROXY_PORT,
            'url'      => sprintf('https://%s:%d', self::PROXY_HOST, self::HTTP_PROXY_PORT),
        ]);
    }
    ...
    public function getProxyIp (ProxyResponse $proxyResponse):string
    {
        return Http::retry(3,100)->withOptions([
            'proxy' => [
                'http' => $proxyResponse->getUrl(),
                'https' => $proxyResponse->getUrl(),
            ],
            'verify' => false,
