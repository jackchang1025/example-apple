...
trait HasBindPhone
{
     ...
     * 尝试进行手机号码绑定操作。
     *
     * 在允许的尝试次数内，重复执行手机号码绑定流程，
     * 包括获取有效手机号码、修改号码状态、执行绑定操作以及绑定成功后的账户状态更新。
     * 如果过程中遇到特定异常，则进行异常处理并决定是否继续下一次尝试。
     * 达到最大尝试次数后仍未成功，则抛出最大重试次数异常。
     *
     * @throws MaxRetryAttemptsException 当超过最大尝试次数仍未能成功绑定手机号码时。
     */
     ...
     * 绑定手机号到用户账户的方法。
     *
     * 此方法通过发送绑定请求到服务端，然后处理响应以获取手机号码ID。
     * 接着尝试获取并验证手机验证码，最后完成手机号与账户的绑定过程。
     *
     * @return void
     * @throws BindPhoneCodeException 如果在响应中未能获取到手机号码ID时抛出异常。
     *
     */
    ...
    protected function handlePhoneException(Throwable $exception): void
    {
        ...
        if ($exception instanceof PhoneNumberAlreadyExistsException || $exception instanceof PhoneException){
